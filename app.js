(() => {
  ...
  let currentBrand = 'any', zoom = .65, toastTimer, restoring = false, printMode = 'labels', precisionUI;
  ...
  function save() { try { localStorage.setItem(STORAGE,JSON.stringify(state)); $('save-status').textContent='✓ 자동 저장됨'; } catch { $('save-status').textContent='저장 공간 부족 · 프로젝트 파일로 저장해 주세요'; } }
  ...
  function resizeLabels() {
    ...
    state.selected=Math.min(state.selected,n-1);state.start=Math.min(state.start,n);
  }
  ...
  function makeLabel(index,printing=false) {
    ...
    const content=document.createElement('span');content.className='label-content';content.textContent=l.text;cell.append(content);
  ...
  $('apply-all').addEventListener('click',()=>{const label={...state.labels[state.selected]};state.labels=state.labels.map(()=>({...label}));renderSheet();save();toast(`${state.labels.length}개의 라벨에 적용했어요.`);});
  ...
  $('import-file').addEventListener('change',async()=>{const file=$('import-file').files[0];if(!file)return;try{if(file.size>2_000_000)throw new Error('too large');const next=migrate(JSON.parse(await file.text()));if(!validProject(next)||!validCalibration(next))throw new Error('invalid');state=next;currentBrand=products.find(p=>p.id===state.product).brand;$('search').value='';$('shape-filter').value='all';$('count-filter').value='all';renderAll();save();toast('프로젝트를 불러왔어요.');}catch{toast('올바른 오마이라벨 프로젝트 파일을 선택해 주세요.');}finally{$('import-file').value='';}});
